...
exports.fetchDeezerPreviewUrlByTrackId = fetchDeezerPreviewUrlByTrackId;
...
function parseItunesYear(releaseDate) {
    if (!releaseDate) {
...
function inferCatalogBucket(trackName, artistName) {
...
function toItunesBattleTrack(item, bucketOverride) {
...
function isReasonableItunesMatch(candidate, trackName, artistName) {
    ...
    const trackMatches = normalizedCandidateTrack.includes(normalizedTargetTrack) ||
        normalizedTargetTrack.includes(normalizedCandidateTrack);
    const artistMatches = normalizedCandidateArtist.includes(normalizedTargetArtist) ||
...
async function fetchItunesPreviewUrl(params) {
    ...
    try {
        ...
        const exactLike = candidates.find((candidate) => Boolean(candidate.previewUrl) && isReasonableItunesMatch(candidate, params.trackName, params.artistName));
...
async function fetchItunesBattleTracks(limit = 120) {
    try {
        ...
        if (topSongsResponse.ok) {
            const topSongsPayload = (await topSongsResponse.json());
...
async function fetchDeezerPreviewUrlByTrackId(trackId) {
