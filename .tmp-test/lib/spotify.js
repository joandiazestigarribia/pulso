...
async function fetchItunesPreviewUrl(params) {
    ...
    try {
        ...
        const exactLike = candidates.find((candidate) => Boolean(candidate.previewUrl) &&
...
function toItunesBattleTrackWithBucket(item, bucketOverride) {
...
function toBattleTrack(track, audioFeatures, bucket = "pop") {
...
async function fetchSpotifyBattleTracks(limit = 40) {
    ...
    try {
        const responses = await Promise.all(SPOTIFY_BUCKET_QUERIES.map(async ({ query, bucket }) => {
