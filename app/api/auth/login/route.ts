...
export async function POST(request: Request) {
  const rateLimit = consumeRateLimit(request, "auth:login")
  ...
  try {
    ...
    const parsedPayload = loginSchema.safeParse(rawPayload)
    ...
    const user = await loginLocalUser(parsedPayload.data)
    ...
    console.error("[auth/login] unexpected error", sanitizeLogData(error))
